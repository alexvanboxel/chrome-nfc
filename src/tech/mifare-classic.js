...
function tagMifareClassic(nfcAdapter, spec, shared) {
  ...
  // input: Uint8Array
  var mif_calc_crc8 = function (input) {
  ...
  // input: Uint8Array
  var mif_calc_crc16 = function (input) {
   ...
   */
  var copy_auth_keys = function (data, dev) {
    for (var i = 0; i < 6; i++) {
  ...
  var read_physical = function (device, phy_block, cnt, cb) {
    var self = this;
    var callback = cb;
    var dev = device;
...
// The callback is called with cb(NDEF Uint8Array).
  ...
  var read_logic = function (device, logic_block, cnt, cb) {
    var self = this;
    var callback = cb;
    var card = new Uint8Array();
...
//         next block continously.
  var write_physical = function (device, block_no, key, all_data, cb) {
    var dev = device;
...
//   ndef: ArrayBuffer. Just ndef is needed. Classic header is handled.
  var write = function (device, ndef, cb) {
    var self = this;
    if (!cb) cb = defaultCallback;
    var callback = cb;
...
// all data blocks.
  var write_logic = function (device, logic_block, all_data, cb) {
    var self = this;
    var callback = cb;
